import { ContentFilter, FilterResponse, ContentFilterResult } from '../filtering/interfaces';
...
export class ContentFilterPipeline {
    ...
    try {
        ...
        try {
          ...
          const result = await Promise.race([
            ...
            new Promise<FilterResponse>((_, reject) => {
              setTimeout(() => reject(new Error(`Filter ${filter.name} timed out`)), filter.timeoutMs || 5000);
            })
          ]);

          // Record metrics
          const filterDuration = performance.now() - filterStartTime;
          this.metrics.recordValue(`filter.latency.${filter.name}`, filterDuration, { tenantId: tenantContext?.tenantId });
          this.metrics.incrementCounter(`filter.result.${filter.name}.${result.result}`, { tenantId: tenantContext?.tenantId });
          ...
          const filterDuration = performance.now() - filterStartTime;
          this.metrics.recordValue(`filter.latency.${filter.name}`, filterDuration, { tenantId: tenantContext?.tenantId });
          this.metrics.incrementCounter(`filter.result.${filter.name}.error`, { tenantId: tenantContext?.tenantId });
      ...
      if (parallelFilters.length > 0) {
        ...
        try {
          ...
          const blockedResult = parallelResults
            .filter(r => r !== null && r.value.result === ContentFilterResult.BLOCKED)
            .sort((a, b) => b.value.confidence - a.value.confidence)[0];

          if (blockedResult) {
            this.logFilterResult(blockedResult.value, blockedResult.filter.name, content);
            return blockedResult.value;
          }
        } catch (error) {
      ...
      this.metrics.recordValue('filter.pipeline.duration', totalDuration, { tenantId: tenantContext?.tenantId });
      this.metrics.incrementCounter('filter.pipeline.result.allowed', { tenantId: tenantContext?.tenantId });
      ...
      const totalDuration = performance.now() - startTime;
      this.metrics.recordValue('filter.pipeline.duration', totalDuration, { tenantId: tenantContext?.tenantId });
      this.metrics.incrementCounter('filter.pipeline.errors', { tenantId: tenantContext?.tenantId });
  ...
  private async logFilterResult(result: FilterResponse, filterName: string, content: string): Promise<void> {
    await ComplianceLogger.log({
